...
> 本文档为 AI 辅助开发提供项目上下文。最后更新: 2026-02-20 / v4.8.6
...
| uvicorn | 0.27.0 | ASGI 服务器（1 worker） |
...
│       │   ├── index.js        # axios 实例、拦截器（401→登录、403→提示、500→提示）
...
- **token_version 机制**: 修改密码/禁用账户时递增 `user.token_version`，旧 token 立即失效
...
- **API 拦截器**: 401→清空 auth store + 自动跳登录（防重复跳转），403→Toast 提示，500→Toast 提示
- **Idle 超时**: 4 小时无操作自动登出（`IDLE_TIMEOUT`）
- **常量集中**: `utils/constants.js` 统一管理菜单、权限、状态映射
- **组合式函数**: `composables/` 封装 format、modal、sort、table、permission 复用逻辑
