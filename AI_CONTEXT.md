...
> 本文档为 AI 辅助开发提供项目上下文。最后更新: 2026-03-14 / v4.24.0
...
- **数据特征**: 单数据库，45 张核心表，支持 SN 码追溯、加权成本核算、应收应付管理（含退货退款闭环）、多账套隔离、员工/部门辅助核算、期末结转、三张财务报表
...
│       │   ├── accounting.py   # AccountSet, ChartOfAccount(含辅助核算标志), AccountingPeriod
...
│       │   ├── invoices.py    # /api/invoices — 发票管理（10端点：销项推送/进项创建/确认/作废/编辑/PDF上传/下载/删除）
...
│       │   │   ├── finance/                  # 财务子组件
│       │   │   │   ├── FinanceOrdersTab.vue   # 970 行 — 订单列表+详情+取消（含付款状态筛选+重置）
...
- **会计模块**（多账套、科目表含员工/部门辅助核算、会计期间、凭证管理、账簿查询）
- **应收应付管理**（7模型、25端点、业务流程自动衔接、退货退款推送、期末凭证生成）
- **发票管理**（销项推送/进项创建/确认/作废/编辑，含明细行，PDF 附件上传/预览/下载/删除）
...
| `/api/invoices` | invoices.py | — | 发票管理（10端点：CRUD+确认/作废+PDF上传/下载/删除） |
