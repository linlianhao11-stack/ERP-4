...
DEFAULT_FEW_SHOTS = [
    ...
    },
    {
        ...
        "queries": [
            ...
            {"title": "采购概况", "sql": "SELECT COUNT(DISTINCT po_no) AS 采购单数, ROUND(SUM(amount),2) AS 采购金额, COUNT(DISTINCT supplier_name) AS 供应商数, SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) AS 已完成, SUM(CASE WHEN status='partial' THEN 1 ELSE 0 END) AS 部分到货, SUM(CASE WHEN status IN ('pending_review','pending','paid') THEN 1 ELSE 0 END) AS 进行中 FROM vw_purchase_detail WHERE purchase_date = CURRENT_DATE - INTERVAL '1 day'"},
