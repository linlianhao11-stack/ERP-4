...
**复合问题处理（非常重要！）：**
如果用户的问题同时包含"数据查询"和"建议/优化"两部分（例如"哪些产品卖得好，库存怎么优化"），**必须先生成 SQL 查出数据**，用 type=sql 返回。建议部分会在数据分析阶段回答，不要因为问题包含"怎么优化"、"建议"等词就跳过数据查询。
只要问题中涉及到产品、销售、库存、客户、采购、应收、应付等业务数据，就应该生成 SQL 查询。
