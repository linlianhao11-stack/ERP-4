...
> 基于 v4.29.0 代码生成，共 **240+** 个端点
> 运行时 Swagger 文档：启动后端后访问 http://localhost:8090/docs（需设置 `DEBUG=true`，生产环境默认关闭）
...
> 路由文件：`app/routers/demo.py`

| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/demo/stats` | 样机统计（总数/在库/借出/逾期/总价值） | `stock_view` |
| GET | `/api/demo/units` | 样机列表（分页，支持 status/search/warehouse_id 筛选） | `stock_view` |
| GET | `/api/demo/units/export` | 导出样机列表 Excel | `stock_view` |
| GET | `/api/demo/units/{unit_id}` | 样机详情（含借出历史+处置记录） | `stock_view` |
| POST | `/api/demo/units` | 新增样机（库存转入/新采购，含仓位） | `stock_edit` |
| PUT | `/api/demo/units/{unit_id}` | 更新样机（成色/备注） | `stock_edit` |
| DELETE | `/api/demo/units/{unit_id}` | 删除样机（仅在库且无借还记录） | `stock_edit` |
| GET | `/api/demo/loans` | 借出记录列表（分页，支持 status/search/日期筛选） | `stock_view` |
| GET | `/api/demo/loans/{loan_id}` | 借出记录详情 | `stock_view` |
| POST | `/api/demo/loans` | 创建借出申请（一机一借校验） | `stock_edit` |
| POST | `/api/demo/loans/{loan_id}/approve` | 审批通过 | `stock_edit` |
| POST | `/api/demo/loans/{loan_id}/reject` | 审批拒绝 | `stock_edit` |
| POST | `/api/demo/loans/{loan_id}/lend` | 确认借出（更新SN码状态） | `stock_edit` |
| POST | `/api/demo/loans/{loan_id}/return` | 归还样机（更新成色+统计） | `stock_edit` |
| POST | `/api/demo/units/{unit_id}/sell` | 转销售（生成订单+应收单+扣库存） | `stock_edit` |
| POST | `/api/demo/units/{unit_id}/convert` | 翻新转良品（转移库存+SN码） | `stock_edit` |
| POST | `/api/demo/units/{unit_id}/scrap` | 报废（扣库存+SN标记） | `stock_edit` |
| POST | `/api/demo/units/{unit_id}/report-loss` | 登记丢失（关闭借出+生成赔偿应收） | `stock_edit` |
