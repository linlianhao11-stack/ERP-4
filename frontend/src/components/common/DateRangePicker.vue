<template>
  <div class="relative" ref="rootRef">
    <!-- 触发器 -->
    <button type="button" class="toolbar-select flex items-center gap-1.5" style="padding-right:10px" @click="toggle">
      ...
      <div v-if="open" ref="panelRef" class="drp-panel" :style="panelStyle">
        ...
        <div class="drp-footer">
          <span class="drp-hint">
            ...
            <template v-else-if="start && end">{{ fmtFull(start) }} – {{ fmtFull(end) }}</template>
          </span>
...
.drp-panel {
  ...
  border-radius: 12px;
  box-shadow: 0 8px 30px oklch(0 0 0 / 0.12), 0 2px 6px oklch(0 0 0 / 0.06);
...
.drp-chip-active { background: var(--primary); color: white; }
...
/* 范围中间（蓝色色带） */
.drp-mid { background: oklch(0.55 0.20 250 / 0.10); }

/* 范围起点（色带右半） */
.drp-range-start {
  background: linear-gradient(to right, transparent 50%, oklch(0.55 0.20 250 / 0.10) 50%);
}
.drp-range-start .drp-num { background: var(--primary); color: white; }

/* 范围终点（色带左半） */
.drp-range-end {
  background: linear-gradient(to left, transparent 50%, oklch(0.55 0.20 250 / 0.10) 50%);
}
.drp-range-end .drp-num { background: var(--primary); color: white; }
...
.drp-picking.drp-range-end .drp-num {
  background: oklch(0.55 0.20 250 / 0.85);
}
