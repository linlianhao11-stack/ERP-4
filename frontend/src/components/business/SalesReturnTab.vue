        ...
        <table class="w-full text-sm">
          <thead class="bg-elevated">
            <tr>
              ...
              <th class="px-2 py-2 text-right">退货成本</th>
          ...
          <tbody class="divide-y">
            ...
            <tr v-for="o in items" :key="o.id" class="hover:bg-elevated">
              <td class="px-2 py-2 font-mono text-[12px]">{{ o.order_no }}</td>
              <td class="px-2 py-2">{{ o.return_date }}</td>
              <td class="px-2 py-2">{{ o.customer_name }}</td>
              <td class="px-2 py-2 text-right">{{ fmtMoney(o.total_amount) }}</td>
              <td class="px-2 py-2 text-right">{{ fmtMoney(o.total_cost) }}</td>
