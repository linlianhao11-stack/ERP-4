          ...
          <tr v-for="r in items" :key="r.id">
            ...
            <td @click.stop>
              <div class="flex gap-1">
                <button @click="viewDetail(r)" class="text-[12px] px-2 py-0.5 rounded-full bg-info-subtle text-primary-active">查看</button>
                <button @click="handleDownloadPdf(r)" class="text-[12px] px-2 py-0.5 rounded-full bg-purple-subtle text-purple-emphasis">PDF</button>
              </div>
            </td>
          </tr>
        </tbody>
          ...
          <div class="modal-body">
            ...
            <template v-else-if="detail">
              <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-[13px] mb-4">
                ...
                <div><span class="text-muted">含税合计：</span><span class="font-medium">{{ detail.total_amount }}</span></div>
                <div><span class="text-muted">不含税：</span>{{ detail.total_amount_without_tax }}</div>
                <div><span class="text-muted">税额：</span>{{ detail.total_tax }}</div>
                  ...
                  <table class="data-table">
                    ...
                    <tbody>
                      <tr v-for="it in detail.items" :key="it.id">
                        ...
                        <td class="text-right">{{ it.tax_inclusive_price }}</td>
                        <td class="text-right">{{ it.tax_exclusive_price }}</td>
