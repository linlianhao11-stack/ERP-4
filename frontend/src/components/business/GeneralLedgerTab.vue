<template>
  <div>
    <div class="card" style="overflow: visible">
      <PageToolbar>
        <template #filters>
          <select v-model="selectedAccountId" class="toolbar-select" @change="query">
            <option :value="null" disabled>选择科目</option>
            <option v-for="a in leafAccounts" :key="a.id" :value="a.id">
              {{ a.code }} {{ a.name }}
            </option>
          </select>
          <input type="month" v-model="startPeriod" class="toolbar-select" @change="query">
          <span class="text-[13px] text-muted">至</span>
          <input type="month" v-model="endPeriod" class="toolbar-select" @change="query">
        </template>
        <template #actions>
