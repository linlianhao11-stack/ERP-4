        ...
        <table class="w-full text-sm">
          <thead class="bg-elevated">
            <tr>
              ...
              <th class="px-2 py-2">原付款单</th>
              <th class="px-2 py-2">退款原因</th>
          ...
          <tbody class="divide-y">
            ...
            <tr v-for="b in items" :key="b.id" class="hover:bg-elevated">
              <td class="px-2 py-2 font-mono text-[12px] max-w-48 truncate" :title="b.bill_no">{{ b.bill_no }}</td>
              <td class="px-2 py-2">{{ b.refund_date }}</td>
              <td class="px-2 py-2">{{ b.supplier_name }}</td>
              <td class="px-2 py-2 text-right">{{ fmtMoney(b.amount) }}</td>
              <td class="px-2 py-2 font-mono text-[12px] max-w-48 truncate" :title="b.original_disbursement_no">{{ b.original_disbursement_no || '-' }}</td>
              <td class="px-2 py-2 max-w-32 truncate">{{ b.reason || '-' }}</td>
...
const suppliers = ref([])
const form = ref({ supplier_id: '', refund_date: new Date().toISOString().slice(0, 10), original_disbursement_id: '', amount: '', reason: '', remark: '' })
...
async function loadList() {
  ...
  const res = await getDisbursementRefundBills(params)
...
function openCreate() {
  form.value = { supplier_id: '', refund_date: new Date().toISOString().slice(0, 10), original_disbursement_id: '', amount: '', reason: '', remark: '' }
  showCreate.value = true
}

async function handleCreate() {
  if (!form.value.supplier_id || !form.value.original_disbursement_id || !form.value.amount) {
    appStore.showToast('请填写供应商、原付款单和退款金额', 'error')
    return
  }
  submitting.value = true
  try {
    await createDisbursementRefundBill(accountingStore.currentAccountSetId, form.value)
