        ...
        >
          ...
          <div class="flex justify-between items-center">
            <div class="flex items-center gap-1.5 text-xs">
              <span :class="s.is_virtual ? 'badge badge-purple' : `badge badge-${s.location_color || DEFAULT_LOCATION_COLOR}`">{{ s.is_virtual ? '寄售' : (s.location_code || '-') }}</span>
          ...
          <tbody class="divide-y">
            ...
            >
              ...
              <td v-if="isColumnVisible('warehouse')" class="px-2 py-2">
                <span :class="row.s.is_virtual ? 'badge badge-purple' : `badge badge-${row.s.location_color || DEFAULT_LOCATION_COLOR}`">{{ row.s.is_virtual ? '寄售' : (row.s.location_code || '-') }}</span>
