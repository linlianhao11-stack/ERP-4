...
| `ai.kd100.key` | 快递 100 Key | AES 加密 |
| `ai.kd100.customer` | 快递 100 Customer | 否 |
| `ai.kd100.secret` | 快递 100 Secret | AES 加密 |
...
环境变量只需一个：`API_KEYS_ENCRYPTION_SECRET`（AES 加密密钥）。
...
通过 Docker entrypoint 初始化脚本自动执行。
