...
所有新增 UI 元素必须完全复用项目现有的组件和 CSS 类，不允许引入新的样式模式。以下是每种 UI 元素的精确实现方式。
...
<div class="toolbar-search-wrapper">
  ...
  <input v-model="searchText" class="toolbar-search" placeholder="搜索单号/客户...">
</div>
```
- 放在 `PageToolbar` 的 `#filters` 插槽中
- 图标使用 `lucide-vue-next` 的 `Search`

### 筛选下拉

```html
<select v-model="filter" @change="loadData()" class="toolbar-select">
...
| 场景 | 类名 | 示例 |
|------|------|------|
| 主操作（新建/提交） | `btn btn-primary btn-sm` | `<button class="btn btn-primary btn-sm">新建</button>` |
| 次操作（导出/取消） | `btn btn-secondary btn-sm` | `<button class="btn btn-secondary btn-sm">导出</button>` |
| 确认操作（收款/收货） | `btn btn-success btn-sm` | `<button class="btn btn-success btn-sm">确认收款</button>` |
| 警告操作（反过账） | `btn btn-warning btn-sm` | `<button class="btn btn-warning btn-sm">反过账</button>` |
| 危险操作（删除） | `btn btn-danger btn-sm` | `<button class="btn btn-danger btn-sm">删除</button>` |
- 放在 `PageToolbar` 的 `#actions` 插槽中
...
<td class="px-3 py-2 text-center">
  <input type="checkbox" v-model="selectedIds" :value="item.id" class="rounded" />
</td>
```
- 使用原生 `<input type="checkbox">` + `class="rounded"`
- 列宽固定 40px
...
```
- 放在 `PageToolbar` 的 `#actions` 插槽中（A2 双视图切换）

### 模态框（弹窗）

```html
<AppModal :visible="showModal" title="标题" @close="showModal = false" width="700px">
  <div class="space-y-4">
    <!-- 内容 -->
  </div>
  <template #footer>
    <div class="flex gap-3">
      <button class="btn btn-secondary flex-1" @click="showModal = false">取消</button>
      <button class="btn btn-primary flex-1" @click="confirm">确认</button>
    </div>
  </template>
</AppModal>
```
- 如不使用 AppModal 组件，则用 `modal-overlay` + `modal-content` + `modal-header` + `modal-body` 结构
- 底部按钮使用 `flex gap-3`，每个按钮 `flex-1` 等宽
...
<SearchableSelect
  ...
  placeholder="选择科目"
  search-placeholder="搜索科目编码或名称..."
/>
```
- 复用现有 `SearchableSelect` 组件
- options 格式：`[{ id, label: '1002 银行存款', sublabel: '资产类' }]`

### 勾单面板（B2 凭证生成）

```html
<AppModal :visible="showBillPicker" title="选择单据生成凭证" @close="close" width="900px">
  <!-- 月份选择器 -->
  <select v-model="selectedPeriod" class="toolbar-select mb-4" @change="loadPendingBills">
    <option v-for="p in periods" :value="p.period_name">{{ p.period_name }}</option>
  </select>
  ...
  <div class="table-container" style="max-height: 400px">
    <table class="w-full text-sm">
      <thead class="bg-elevated sticky top-0">
        <tr>
          <th class="px-3 py-2 text-center" style="width: 40px">
            <input type="checkbox" v-model="selectAllBills" class="rounded" />
          </th>
          <th class="px-3 py-2 text-left">单据号</th>
          <th class="px-3 py-2 text-left">类型</th>
          <th class="px-3 py-2 text-left">客户/供应商</th>
          <th class="px-3 py-2 text-right">金额</th>
          <th class="px-3 py-2 text-left">日期</th>
        </tr>
      </thead>
      <tbody class="divide-y">
        <tr v-for="b in pendingBills" class="hover:bg-elevated">
          <td class="px-3 py-2 text-center">
            <input type="checkbox" v-model="selectedBillIds" :value="b.id" class="rounded" />
          </td>
          <!-- ... -->
        </tr>
      </tbody>
  ...
  <template #footer>
    <div class="flex items-center justify-between w-full">
      ...
      <div class="flex gap-3">
        <span class="text-sm text-muted self-center">将生成 {{ previewCount }} 张凭证</span>
        <button class="btn btn-secondary btn-sm" @click="close">取消</button>
        <button class="btn btn-primary btn-sm" @click="generate" :disabled="!selectedBillIds.length">确认生成</button>
      </div>
    </div>
  </template>
</AppModal>
...
```html
<AppModal :visible="showInvoicePush" title="推送销项发票" @close="close" width="800px">
  <!-- 客户开票信息 -->
  <div class="space-y-3 mb-4">
    <h4 class="text-sm font-medium text-secondary">客户开票信息</h4>
    <div class="grid form-grid grid-cols-2 gap-3">
      <div><label class="label">税号</label><input class="input" v-model="taxId" /></div>
      <div><label class="label">开户行</label><input class="input" v-model="bankName" /></div>
    </div>
  </div>

  <!-- 货物明细 -->
  <h4 class="text-sm font-medium text-secondary mb-2">货物明细</h4>
  <div class="table-container">
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y"><!-- 行 --></tbody>
    </table>
  </div>

  <template #footer>
    <div class="flex gap-3">
      <button class="btn btn-secondary flex-1" @click="close">取消</button>
      <button class="btn btn-primary flex-1" @click="pushInvoice">确认推送</button>
    </div>
  </template>
</AppModal>
```
...
1. **颜色**：只用 CSS 变量 `var(--primary)`、`var(--text)`、`var(--border)` 等，禁止硬编码 hex 值
2. **图标**：统一使用 `lucide-vue-next`，`:size="14"` 用于按钮内图标，`:size="16"` 用于独立图标
3. **响应式**：桌面表格用 `hidden md:block`，移动卡片用 `md:hidden`
4. **间距**：使用 Tailwind 工具类 `gap-*`、`px-*`、`py-*`、`space-y-*`
5. **表单**：`.label` + `.input` 组合，两列用 `grid form-grid grid-cols-2 gap-3`
6. **空状态**：`<div class="p-8 text-center text-muted text-sm">暂无数据</div>`
7. **状态徽章**：使用 `StatusBadge` 组件或 `badge badge-*` 类
8. **分页**：`flex items-center justify-center gap-2 py-3` + `btn btn-secondary btn-sm`
...
1. **单文件组件不超过 300 行**：超过即需拆分为子组件。VoucherPanel.vue 当前已较大，本次改动涉及双视图、批量操作、搜索导出，必须拆分。
2. **一个组件一个职责**：不要把勾单面板、发票推送弹窗、批量操作逻辑全塞在父组件里。
3. **及时重构**：项目处于开发阶段，不存在生产兼容负担。需要重构就直接重构，不硬写。

### 计划 A 组件拆分方案

VoucherPanel.vue 当前承载凭证列表 + 详情弹窗 + 工作流操作，本次新增双视图 + 批量操作 + 搜索导出，必须拆分：

```
VoucherPanel.vue                 ← 容器，管理视图切换和状态
├── VoucherListView.vue          ← 凭证视图（每行一张凭证）
├── VoucherEntryListView.vue     ← 分录视图（每行一条分录）— 新增
├── VoucherDetailModal.vue       ← 凭证详情/编辑弹窗（已有逻辑抽出）
└── VoucherBatchActions.vue      ← 批量操作逻辑（提交/审核/过账）— 新增
```

### 计划 B 组件拆分方案

勾单生成面板是独立功能，不要内联到 ReceivablePanel / PayablePanel：

```
ReceivablePanel.vue
├── ...existing tabs...
└── VoucherGenerateModal.vue     ← 通用勾单生成弹窗（AR/AP 共用，通过 props 区分）

PayablePanel.vue
└── 复用同一个 VoucherGenerateModal.vue
```
...
```
InvoicePushModal.vue             ← 通用发票推送弹窗（销项/进项通过 props 区分）
├── 开票信息表单区
└── 货物明细表格区
```
...
1. **Service 层保持单一职责**：新增的合并生成逻辑如果导致 `ar_service.py` / `ap_service.py` 过长，提取为独立函数或类方法
2. **复用而非复制**：AR 和 AP 的勾单合并逻辑高度对称，提取公共辅助函数避免 copy-paste
3. **路由层保持薄**：路由只做参数校验和权限检查，业务逻辑全部在 service 层
