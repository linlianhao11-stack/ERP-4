...
_SECRET = os.environ.get("API_KEYS_ENCRYPTION_SECRET", "")


def _get_fernet() -> Fernet:
    """从环境变量派生 Fernet 密钥（32 字节 base64）"""
    if not _SECRET:
...
def _build_ai_dsn() -> str:
    ...
    password = AI_DB_PASSWORD or "ai_readonly_default"
    return f"postgresql://erp_ai_readonly:{password}@{host_port}/{dbname}"
...
const checkStatus = async () => {
  try {
    ...
    if (data.available) {
      // 加载预设问题
      try {
        const { data: cfg } = await getAiConfig()
        presetQueries.value = cfg['ai.preset_queries'] || []
      } catch { /* 非 admin 可能无权读 ai-config，忽略 */ }
    }
  } catch {
