...
- Modify: `backend/app/routers/receivables.py:190` (清理 bill_type 返回字段)
- Modify: `backend/app/routers/payables.py:189` (清理 bill_type 返回字段)
...
**Step 11b: 清理收款单/付款单列表 API 中的 bill_type 返回字段**

- `receivables.py:190`：列表端点返回数据中移除 `bill_type` 字段
- `payables.py:189`：同上
...
行 327 的状态文案从 `已退款给客户 / 形成在账资金` 改为 `需要退款（待财务确认） / 已转为在账资金`。
...
- Modify: `frontend/src/views/CustomersView.vue:336`
- Modify: `frontend/src/components/business/finance/FinanceOrdersTab.vue:110`
- Modify: `frontend/src/components/business/purchase/PurchaseOrderDetail.vue:33`
- Modify: `frontend/src/components/business/purchase/PurchaseReturnTab.vue:72,120`
- Modify: `frontend/src/views/GuideView.vue:737`
...
所有展示退货退款状态的位置，将文案从旧语义更新为新语义：

- `CustomersView.vue:336`：`已退款给客户 / 形成在账资金` → `需要退款（待财务确认） / 已转为在账资金`
- `FinanceOrdersTab.vue:110`：订单列表中的退款状态显示，同上
- `PurchaseOrderDetail.vue:33`：`已退款 / 转为在账资金` → `需要退款（待确认） / 已转为在账资金`
- `PurchaseReturnTab.vue:72,120`：退货单列表和详情中的退款方式文案更新
- `GuideView.vue:737`：使用指南中「确认是否已退款」→ 「选择是否需要退款」
...
```bash
git commit -m "fix(frontend): 清理旧退款标签 + 更新全局退款状态文案"
```
